...
void ExchangeStation::preTreatment(Mat src, Mat &out)
{
    ...
    //imshow("out12", out12);// waitKey(0);
    //imshow("notout12", notout12);// waitKey(0);
    out= out12+notout12;
...
int ExchangeStation::getPoint(Mat & src,Mat origin)
{
    ...
    */
    ...
    corners.push_back(left_up.center);
    corners.push_back( better_right_on_point);
    corners.push_back(right_down.center);
    corners.push_back(left_down.center);
    return 1;
}


bool ExchangeStation::isRectOutOfMat( const cv::Mat& mat,const cv::Rect& rect)
{
...
{
    if(left_up.size.empty()||right_down.size.empty()||left_down.size.empty())
    {
        return -1;
    }
    ...
    if( isRectOutOfMat(image, left_up.boundingRect())||
        isRectOutOfMat(image, mask)||
        isRectOutOfMat(image, right_down.boundingRect())||
        isRectOutOfMat(image, left_down.boundingRect()))
    {
        return -1;
    }
    Rect station[4] = { left_up.boundingRect(), mask , right_down.boundingRect() , left_down.boundingRect()};
    Mat ROI_img1(image, left_up.boundingRect());
    Mat ROI_img2(image, mask);
    Mat ROI_img3(image, right_down.boundingRect());
    Mat ROI_img4(image, left_down.boundingRect());
    if (ROI_img1.type() != CV_8UC1) {
        ROI_img1.convertTo(ROI_img1, CV_8UC1);
    }
    if (ROI_img2.type() != CV_8UC1) {
        ROI_img2.convertTo(ROI_img2, CV_8UC1);
    }
    if (ROI_img3.type() != CV_8UC1) {
        ROI_img3.convertTo(ROI_img3, CV_8UC1);
    }
    if (ROI_img4.type() != CV_8UC1) {
        ROI_img4.convertTo(ROI_img4, CV_8UC1);
    }
    Mat rois[4] = { ROI_img1 , ROI_img2 , ROI_img3 , ROI_img4 };
    //imshow("0",rois[0]);
    //imshow("1",rois[1]);
    //imshow("2",rois[2]);
    //imshow("3",rois[3]);
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        Mat roi(image,station[i]);
        cv::goodFeaturesToTrack(roi, corners, 1, 0.8, 1, rois[i]);//
    }

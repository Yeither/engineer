...
int main()
{
    ...
    while (1)
    {
        ...
        src.copyTo(dst);
        a.judgeStation(dst);
        vector<Point_<float>> points_float;
        for (const auto& point : a.corners) {
            Point_<float> p(point.x, point.y);
            points_float.push_back(p);
        }
        an.Solver("/home/zhouhb/桌面/exchangefuben/camera_params.xml", 1,a.exchangeStation1,dst,points_float);
        //imshow("111", dst);
        a.corners.clear();
        waitKey(1);
